  ...
  methods: {
    ...
    initTinymce() {
      window.tinymce.init({
        ...
        fontsize_formats: '6px 8px 9px 10px 11px 12px 14px 16px 18px 20px 22px 24px 26px 28px 36px 48px 72px',
        end_container_on_empty_block: true,
        powerpaste_word_import: 'merge',
        paste_data_images: true,
        code_dialog_height: 450,
    ...
    uploadSuccessCBK(arr) {
      arr.forEach(v => {
        window.tinymce.get(this.tinymceId).insertContent(`<p><a href="${v.url}" target="${this.default_link_target}">${v.name}</a></p>`)
      })
    },
